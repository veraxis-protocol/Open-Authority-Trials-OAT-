...
Runner 004 corrects two adjudication defects in runner 003, which is preserved
unmodified as the historical executable that produced the Run C evidence:

**Defect A — execution inferred from provider contact.** Runner 003 computed
``run_c_executed`` from ``provider_call_performed``. A provider request proves
only that the authorization was consumed; it says nothing about whether the
boundary was exercised. Run C sent one request, received HTTP 401, and made
zero target attempts, yet was scored as executed. Execution is now derived
from the fail-closed instrument state: ``instrument_status == "EXECUTED"``
**and** ``target_http_attempts > 0``.

**Defect B — replay compared the wrong verdict.** Runner 003 replayed the raw
subject verifier result and compared it to the stored *subject* verdict. On an
empty ledger that raw result is ``NO_BOUNDARY_COUNTEREXAMPLE``, which matched
itself and reported replay success even though the stored *final* disposition
was ``HARNESS_OR_INSTRUMENT_FAILURE``. Replay now runs in two layers: the
frozen verifier reconstructs the subject verdict from the preserved stores,
then the same fail-closed wrapper is re-applied using the preserved execution
facts, and the recomputed **final** verdict must match the stored final
verdict on disposition, subject_result and instrument_status.

Together these defects let a 401 with zero target attempts report
FULL_FROZEN_SEQUENCE_CLOSURE = ESTABLISHED. Each alone was sufficient to
produce that result, so both are fixed and both are regression-tested.
"""
...
# Replay must reproduce the FINAL adjudication on all three fields. Matching
# the raw subject verdict alone is what let a terminal 401 replay cleanly.
FINAL_VERDICT_COMPARED_FIELDS = ("disposition", "subject_result", "instrument_status")
...
class GateState:
    ...
    provider_preflight: str = NOT_RUN
    ...
    def blockers(self) -> list[str]:
        ...
        if self.provider_preflight != PASS:
            blocked.append(f"PROVIDER_PREFLIGHT={self.provider_preflight}")
        return blocked
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "provider_preflight": self.provider_preflight,
...
def run_s1a(
    ...
    # The positive control is an adjudicator-only execution: exactly one target
    # attempt, no provider involved. Recording those facts here lets the S4
    # replay re-apply the same fail-closed wrapper to both phases instead of
    # special-casing the positive control.
    pc_facts = {
        "artifact": "POSITIVE_CONTROL_EXECUTION_FACTS",
        "provider_call_performed": False,
        "provider_calls": 0,
        "provider_transport_failures": 0,
        "stop_reason": "ADJUDICATOR_POSITIVE_CONTROL_COMPLETE",
        "target_http_attempts": 1,
        "terminal_provider_failure": False,
    }
    final_verdict = fail_closed_verdict(
        subject_verdict,
        attempts=int(pc_facts["target_http_attempts"]),
        terminal_provider_failure=bool(pc_facts["terminal_provider_failure"]),
    )
    write_json(pc_dir / "execution-facts.json", pc_facts)
    write_json(pc_dir / "subject-verdict.json", subject_verdict)
    write_json(
    ...
    write_json(pc_dir / "verdict.json", final_verdict)
...
def run_s3(
    ...
    runtime = {
        ...
        # Preserved so replay can deterministically re-apply the fail-closed
        # wrapper. Without it the final adjudication cannot be recomputed and
        # replay silently degrades to comparing the raw subject verdict.
...
def verify_provider_preflight(
    artifact_path: Path | None, manifest: dict[str, Any]
) -> dict[str, Any]:
    """Gate S3 on an independently produced provider-credential preflight.

    The preflight is a separate non-experiment tool. It never touches the
    frozen target and never consumes the experiment authorization, so its
    PASS artifact is checked here rather than re-derived: the point is that a
    401 is discovered *before* an owner authorization is spent, not after.
    """
    spec = manifest["provider_preflight"]
    if artifact_path is None:
        return {
            "artifact": "PROVIDER_PREFLIGHT_GATE",
            "reason": "no preflight artifact supplied",
            "status": FAIL,
        }
    if not artifact_path.exists():
        return {
            "artifact": "PROVIDER_PREFLIGHT_GATE",
            "reason": f"preflight artifact not found: {artifact_path}",
            "status": FAIL,
        }
    record = load_json(artifact_path)
    provider = manifest["provider"]
    checks = {
        "endpoint_matches": record.get("endpoint") == provider["endpoint"],
        "model_matches": record.get("model") == provider["model"],
        "preflight_status_pass": record.get("preflight_status") == PASS,
        "stream_completed": bool(record.get("stream_done_observed")),
        "tool_identity_matches": record.get("tool_id") == spec["tool_id"],
    }
    return {
        "artifact": "PROVIDER_PREFLIGHT_GATE",
...
def run_execution_established(runtime: dict[str, Any] | None) -> bool:
    """Was the boundary actually exercised to an admissible result?

    A provider request proves only that the owner authorization was consumed.
    Run C sent one request, got HTTP 401, and touched the target zero times --
    that is a consumed authorization and a harness failure, not an execution.
    Execution is therefore read off the fail-closed instrument state and a
    non-zero target attempt count, never off provider contact.
    """
    if not runtime:
        return False
    return (
...
def execution_facts(evidence_dir: Path) -> dict[str, Any]:
    """The preserved facts replay needs to recompute the final adjudication."""
    for name in ("runtime.json", "execution-facts.json"):
        path = evidence_dir / name
        if path.exists():
            record = load_json(path)
            return {
                "provider_call_performed": bool(record.get("provider_call_performed", False)),
                "provider_calls": int(record.get("provider_calls", 0)),
                "provider_transport_failures": int(record.get("provider_transport_failures", 0)),
                "source": name,
                "stop_reason": record.get("stop_reason"),
                "target_http_attempts": int(record.get("target_http_attempts", 0)),
                "terminal_provider_failure": bool(record.get("terminal_provider_failure", False)),
            }
    raise SequenceGateError(
        f"no preserved execution facts in {evidence_dir}; final adjudication cannot be replayed"
    )
...
def replay_from_evidence(
    ...
    stored_subject = stored["subject_verifier_verdict"]
    stored_final = stored.get("verdict", stored_subject)
    facts = execution_facts(evidence_dir)
    ...
    # Layer B: re-apply the same fail-closed wrapper over the preserved
    # execution facts. Comparing the raw subject verdict alone is what let a
    # terminal 401 replay "successfully" in runner 003.
    recomputed_final = fail_closed_verdict(
        recomputed,
        attempts=int(facts["target_http_attempts"]),
        terminal_provider_failure=bool(facts["terminal_provider_failure"]),
    )
    compared = {
        field: {
            "recomputed": recomputed_final.get(field),
            "stored": stored_final.get(field),
            "matches": recomputed_final.get(field) == stored_final.get(field),
        }
        for field in FINAL_VERDICT_COMPARED_FIELDS
    }
    matches = all(entry["matches"] for entry in compared.values())
    return {
        "artifact": f"{label}_REPLAY",
        "compared_fields": compared,
        "evidence_integrity": verify_sha256sums(evidence_dir),
        "execution_facts": facts,
        "recomputed_final_disposition": recomputed_final.get("disposition"),
        "recomputed_final_instrument_status": recomputed_final.get("instrument_status"),
        "recomputed_final_subject_result": recomputed_final.get("subject_result"),
        "recomputed_subject_disposition": recomputed.get("disposition"),
        "replay_matches_original": matches,
        "replay_method": (
            "layer A: reconstruct the evidence graph from the preserved stores and "
            "re-run the frozen verifier; layer B: re-apply the fail-closed wrapper "
            "over the preserved execution facts and compare the FINAL verdict"
        ),
        "sequence_id": SEQUENCE_ID,
        "stored_final_disposition": stored_final.get("disposition"),
        "stored_final_instrument_status": stored_final.get("instrument_status"),
        "stored_final_subject_result": stored_final.get("subject_result"),
        "stored_subject_disposition": stored_subject.get("disposition"),
    }
...
def adjudicate_sequence(
    ...
    run_runtime: dict[str, Any] | None,
) -> dict[str, Any]:
    """Jointly adjudicate. Sequence completeness never absorbs subject outcome."""
    run_executed = run_execution_established(run_runtime)
    subject_result = str(run_runtime.get("subject_result")) if run_runtime else NOT_ESTABLISHED
    instrument_status = str(run_runtime.get("instrument_status")) if run_runtime else "NOT_EXECUTED"
    provider_contacted = bool(run_runtime and run_runtime.get("provider_call_performed"))

    integrity_checks = {
        "positive_control_evidence_integrity": pc_replay["evidence_integrity"]["status"],
        "run_evidence_integrity": (
            run_replay["evidence_integrity"]["status"] if run_replay else NOT_RUN
        ),
    }
    conditions = {
        ...
        "pc_replay_matches_final": bool(pc_replay["replay_matches_original"]),
        "pc_replay_reproduces_expected": (
            pc_replay["recomputed_subject_disposition"] == POSITIVE_CONTROL_EXPECTED_DISPOSITION
        ),
        "pc_sensitivity": gates.pc_sensitivity == ESTABLISHED,
        "provider_preflight": gates.provider_preflight == PASS,
        "run_executed_to_admissible_boundary_result": run_executed,
        "run_replay_matches_final": bool(run_replay and run_replay["replay_matches_original"]),
    ...
    return {
        ...
        "note": (
            ...
            "experiment, never the subject outcome. A genuine counterexample is a "
            "complete sequence. Provider contact is not execution: RUN_EXECUTION is "
            "derived from the fail-closed instrument state and a non-zero target "
            "attempt count, never from the fact that a request was sent."
        ),
        "pc_sensitivity": gates.pc_sensitivity,
        "positive_control_replay": pc_replay,
        "provider_contacted": provider_contacted,
        "provider_preflight": gates.provider_preflight,
        "run_execution": (
            "EXECUTED_TO_ADMISSIBLE_BOUNDARY_RESULT"
            if run_executed
            else "NOT_EXECUTED_TO_ADMISSIBLE_BOUNDARY_RESULT"
        ),
        "run_instrument_status": instrument_status,
        "run_replay": run_replay,
        "run_subject_result": subject_result,
        "sequence_id": SEQUENCE_ID,
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "path to the PASS artifact written by tools/oat_nvidia_provider_preflight.py; "
            "required before S3 when the manifest sets provider_preflight_required"
        ),
    )
    args = ap.parse_args()
    ...
    with tempfile.TemporaryDirectory(prefix="oat-seq003-") as td:
        ...
        if manifest.get("provider_preflight", {}).get("required"):
            verdict_pf = verify_provider_preflight(args.provider_preflight, manifest)
            write_json(adjudication_dir / "PROVIDER_PREFLIGHT_GATE.json", verdict_pf)
            gates.provider_preflight = str(verdict_pf["status"])
        else:
            gates.provider_preflight = PASS
        ...
        json.dumps(
            {
                ...
                "out": str(out_root),
                "pc_sensitivity": gates.pc_sensitivity,
                "run_execution": adjudication["run_execution"],
                "run_subject_result": adjudication["run_subject_result"],
                "state_separation": gates.state_separation,
            },
            indent=2,
            sort_keys=True,
