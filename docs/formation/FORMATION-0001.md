...
## Addendum — 2026-09-15, work order OAT-GITHUB-PUBLISH-001

The licensing decision recorded above as "mine, pending owner review" was
reviewed and **superseded by the owner**. The repository now carries three
distinct grants:

- **Code** (`oat/`, `tests/`, build and CI config): Business Source License
  1.1. The Change Date is 2030-09-15 and the Change License is Apache
  License, Version 2.0. BSL 1.1 is not an Open Source license.
- **Documentation, methodology, specifications, schemas, Formation Records**
  (including this file): CC BY 4.0.
- **Names, marks, and claim-bearing designation**: reserved, no grant.

The licensing change is documentary only. It touched `LICENSE.md`,
`LICENSE-DOCS.md`, `TRADEMARKS-AND-STANDING.md`, `README.md`,
`pyproject.toml`, and added `tests/test_licensing.py`. No file under `oat/`
was modified, so RB-001 computational behavior and every recorded evidence
digest are unchanged — verified by re-running the full gate and confirming the
reference run regenerated byte-identically.

The constitutional state is untouched by licensing:
